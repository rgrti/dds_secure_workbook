...
ECHO      openssl ecparam -name secp384r1 -genkey -noout -out private\permissionscakey.pem
openssl ecparam -name secp384r1 -genkey -noout -out private\permissionscakey.pem
ECHO.

ECHO generating certificate signing request (identca.csr) based on private key and openssl.cnf
ECHO      openssl req -new -sha384 -key private\permissionscakey.pem -out ca.csr -config openssl.cnf
openssl req -new -sha384 -key private\permissionscakey.pem -out ca.csr -config openssl.cnf
ECHO.

ECHO generating the permissions CA certificate (permcacert.pem) based on the .csr certificate request and private key
ECHO      openssl x509 -req -sha384 -days 3650 -in ca.csr -text -signkey private\permissionscakey.pem -out permissionscacert.pem
openssl x509 -req -sha384 -days 3650 -in ca.csr -text -signkey private\permissionscakey.pem -out permissionscacert.pem
ECHO.
