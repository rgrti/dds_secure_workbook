...
ECHO      openssl ecparam -name secp384r1 -genkey -noout -out private\cakey.pem
openssl ecparam -name secp384r1 -genkey -noout -out private\cakey.pem
ECHO.

ECHO generating certificate signing request (ca.csr) based on private key and openssl.cnf
ECHO      openssl req -new -sha384 -key private\cakey.pem -out ca.csr -config openssl.cnf
openssl req -new -sha384 -key private\cakey.pem -out ca.csr -config openssl.cnf
ECHO.

ECHO generating the CA certificate (cacert.pem) based on the .csr certificate request and private key
ECHO      openssl x509 -req -sha384 -days 3650 -in ca.csr -text -signkey private\cakey.pem -out cacert.pem
openssl x509 -req -sha384 -days 3650 -in ca.csr -text -signkey private\cakey.pem -out cacert.pem
ECHO.
